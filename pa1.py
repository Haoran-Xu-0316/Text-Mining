...
'''def zhankai():
    ...
    for i in range(1, 16):
        try:
            zhankai_xpath = pre + str(i) + ']/div/div[3]/div/a'
            driver.find_element(By.XPATH, zhankai_xpath).click()
        except:
            try:
                zhankai_xpath = pre + str(i) + ']/div/div[4]/div/a'
                driver.find_element(By.XPATH, zhankai_xpath).click()
...
def login():
    driver.find_element(By.XPATH, r'//*[@id="top-nav"]/div/div[2]/span[1]/a[1]').click()
    time.sleep(15) # 登录
...
def get_comment():
    ...
    for i in range(1, len(name_texts)):
        clist = []
...
if __name__ == '__main__':
    ...
    driver.get('https://www.dianping.com/shop/Ha49nhkXQ4yZtbJ8/review_all')
    time.sleep(5)
    login()
    driver.get('https://www.dianping.com/shop/Ha49nhkXQ4yZtbJ8/review_all')

    # 获取密码表
    css_link = driver.find_element(By.XPATH,r'/html/head/link[4]').get_attribute('href')
    ...
    for i in range(0, page):
        ...
        for j in range(0, 15):
            try:
                ...
                info_table.to_csv('秋季语寿司·轻食(顺达店)' + name + '.csv', encoding='gbk')
        print('已完成' + str(i + 1) + '页')
        try:
            next_page()
        except:
